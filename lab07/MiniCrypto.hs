...
test1 = testOne 1 $ testVal (take 42 $ myCycle xs) (take 42 $ cycle xs) "myCycle" 1
  where xs = [1,2,3,4]
myCycle :: [a] -> [a]
myCycle a = iter a where iter a = a ++ iter (a)
...
arithmetic a b = iterate (\ x -> x + b) a
geometric :: Num a => a -> a -> [a]
geometric a b = iterate (\ x -> x * b) a
...
randoms :: Int -> [Word8]
randoms a = map (fromIntegral . fst) (iterate (next . snd) $ next $ mkStdGen a)
...
wordToAlpha a = chr ((mod (fromIntegral a) 26) + 97)
{-
    5. (1p)
...
randomAlphaKey key = map wordToAlpha $ randoms key
{-
  6. (3p)
