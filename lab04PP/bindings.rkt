...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ((segLen (get-line-segment))
         (startP (get-start-point segLen))
         (endP (get-end-point segLen)))
    (sqrt (+ (expt ( - (get-x startP) (get-x endP)) 2) (expt ( - (get-y startP) (get-y endP)) 2))) 
    )
  )
...
(define (compute-perimeter points)
  (let P ((curP points))
    (if (null? (cdr curP))
        (distance (car points) (car curP))
        (+ (distance (car curP) (cadr curP)) (P (cdr curP))))))
...
(define (3-sequence-max numbers separator)
  (let*-values (((separate) (λ (x) (not (= x separator))))
                ((seq1 s1) (splitf-at numbers separate))
                ((seq2 s2) (splitf-at (cdr s1) separate))
                ((seq3 s3) (splitf-at (cdr s2) separate)))
    (max (max (sumL seq1) (sumL seq2)) (sumL seq3))))
