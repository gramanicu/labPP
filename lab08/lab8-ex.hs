...
lengthV (V x y z) = abs$sqrt$x^2 + y^2 + z^2

normalizeV :: Vector -> Vector
normalizeV (V x y z) = (V (x / (lengthV (V x y z))) (y / (lengthV (V x y z))) (z / (lengthV (V x y z))))
...
orthogonalV (v : []) = True
orthogonalV (elem : rest)
  | all(==0) (map (dotV elem) rest) == True = orthogonalV rest
  | otherwise = False
...
{-
 2. (4p) Liste imbricate
  ...
  (1p) flatten
-}
data SList a = UndefinedSList | Value a | List [SList a] deriving Show

emptySList :: SList a
emptySList = List []

consElem :: a -> SList a -> SList a
consElem a (Value b) = List [Value a, Value b]
consElem a (List b) = List (Value a :b)

consList :: SList a -> SList a -> SList a
consList a (Value b) = List [a, Value b] 
...
deepEqual (Value a) (Value b) = (a == b)
deepEqual (List a) (List b)
  | (length a) == (length b) = and (zipWith deepEqual a b)
  | otherwise              = False
